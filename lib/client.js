...
const { topicParser, errorParser, showTopicParser } = require('./parsers');
...
class CoreBay {
    ...
    async showTopic(id) {
      ...
      const body = await this._call('GET', query).then(res => res.body());

      const errorMessage = errorParser(body);

      if (errorMessage) throw new NotExistsTopicError(errorMessage);

      const data = showTopicParser(body);
