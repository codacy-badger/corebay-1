...
class Parser {
  ...
  get authorData() {
    return this._authorData('td > .postdetails', '#profilename');
  }

  _authorData(elementData, elementName) {
    const { $ } = this;
    const author = {
      name: this.$(elementName).text()
    };
    ...
    const groupId = USER_GROUPS[author.group];

    return { ...author, groupId };

  }

  get pinned() {
    return this.$('[name="CODE"] [value="15"], [value="16"]').val();
  }
  ...
  get errorNews() {
    return this.$('td[align="center"]').eq(0).text();
  }
  ...
  get topicData() {
    ...
    const posts = $('.ipbtable[cellspacing="1"]')
      .map((_, element) => {
        ...
        const elAuthorData = $$.find('td > .postdetails');
        const elAuthorName = $$.find('b > span > span');

        const author = this._authorData(elAuthorData, elAuthorName);
